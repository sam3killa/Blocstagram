...
- (void) addFiltersToQueue {
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        if (chromeFilter) {
            ...
            if (boomFilter) {
                [boomFilter setValue:result forKeyPath:kCIInputImageKey];
                result = boomFilter.outputImage;
            }
            
            [self addCIImageToCollectionView:result withFilterTitle:NSLocalizedString(@"Coolness", @"Coolness Filter")];
        }
    }];
